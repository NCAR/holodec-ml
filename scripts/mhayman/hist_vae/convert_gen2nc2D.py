...
for idx,one_hologram in enumerate(training_data):
    ...
    if y_out['d'].size <= max_count:
        ...
        dz_hist = np.histogram2d(y_out['d'],y_out['z'],bins=(dbins,zbins)) 
        ...
        dz_hist_lst +=[dz_hist[0][np.newaxis,:,:]]
...
ds_dct['dz_hist'] = xr.DataArray(np.concatenate(dz_hist_lst,axis=0),dims=('hologram_number','d_bin_centers','z_bin_centers'),
                                coords={'hologram_number':ds_dct['hologram_number'],'d_bin_centers':ds_dct['d_bin_centers'],'z_bin_centers':ds_dct['z_bin_centers']},
                                attrs={'description':'histogram of particle diameter and z position'})
...
if 'validation' in dataFile:
    isplt = vae_ds.dims['hologram_number']//2
    # split validation data file into test and validation
    vae_vld_ds = vae_ds.isel(hologram_number=slice(None,isplt))
    vae_tst_ds = vae_ds.isel(hologram_number=slice(isplt,None))
    
    vae_vld_ds.to_netcdf(savePath+dataFile.replace('.pkl',f'2D_le{max_count}.nc'))
    vae_tst_ds.to_netcdf(savePath+dataFile.replace('validation','test').replace('.pkl',f'2D_le{max_count}.nc'))
    ...
    vae_ds.to_netcdf(savePath+dataFile.replace('.pkl',f'2D_le{max_count}.nc'))
    vae_ds.close()
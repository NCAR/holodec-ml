...
with xr.open_dataset(paths['load_data']+settings['data_file'],chunks={'hologram_number':settings['h_chunk']}) as ds:
    ...
    if separate_files:
        ...
        for lab_var in label_variable:
            ...
            train_lab_lst += [ds[lab_var].sel(d_bin_centers=slice(dmin,None)).sum(dim=('d_bin_centers')).expand_dims('dense_output')]
        ...
        with xr.open_dataset(paths['load_data']+settings['test_file'],chunks={'hologram_number':settings['h_chunk']}) as ds_test:
            ...
            test_lab_lst = []
            for lab_var in label_variable:
                test_lab_lst += [ds_test[lab_var].sel(d_bin_centers=slice(dmin,None)).sum(dim=('d_bin_centers')).expand_dims('dense_output')]
        ...
        with xr.open_dataset(paths['load_data']+settings['validation_file'],chunks={'hologram_number':settings['h_chunk']}) as ds_valid:
            ...
            valid_lab_lst = []
            for lab_var in label_variable:
                valid_lab_lst += [ds_valid[lab_var].sel(d_bin_centers=slice(dmin,None)).sum(dim=('d_bin_centers')).expand_dims('dense_output')]

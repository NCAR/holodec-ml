...
from tensorflow.keras.layers import Input, Conv2D, Dense, Flatten, Activation, MaxPool2D, SeparableConv2D, UpSampling2D, concatenate, Conv2DTranspose, Embedding, Attention
...
dim_embedding_size = Integer(low=settings['embedding_size'][0],
                        high=settings['embedding_size'][1],
                        name="embedding_size")
...
default_parameters = [  settings['learning_rate'][2], 
                        ...
                        settings['embedding_size'][2],]




def create_model(learning_rate, num_dense_layers,num_input_nodes,
                 num_dense_nodes,activation,adam_decay,input_shape,embedding_size):
    ...
    encode_input = Input(shape=input_shape) # input_shape=scaled_train_input.shape[1:]

    # flatten the convolution output for Dense Layers
    flat_input = Flatten()(encode_input)
    token_embedding = Embedding(input_dim=np.prod(input_shape), output_dim=embedding_size)
    # Query embeddings of shape [batch_size, Tq, dimension].
    query_embeddings = token_embedding(encode_input)
    # Value embeddings of shape [batch_size, Tv, dimension].
    value_embeddings = token_embedding(encode_input)

    # Query-value attention of shape [batch_size, Tq, filters].
    query_value_attention = Attention()(    
    [query_embeddings, value_embeddings])

    attention_output = Flatten()(query_value_attention)

    # add dense layers
    dense_out = []
    dense_out+=[Dense(num_dense_nodes,activation=activation)(attention_output)]
    for _ in range(num_dense_layers):
        dense_out += [Dense(num_dense_nodes,activation=activation)(dense_out[-1])]

    # add the output layer
    out = Dense(np.prod(scaled_train_labels.shape[1:]),activation=settings['output_activation'])(dense_out[-1])

    adam = Adam(lr=learning_rate, decay= adam_decay)
    mod = Model(encode_input, out)
...
mod = create_model(gp_result.x[0],gp_result.x[1],gp_result.x[2],gp_result.x[3],gp_result.x[4],gp_result.x[6],scaled_train_input.shape[1:],gp_result.x[7])
...
for m in settings.get('moments',[0,1,2,3]):
    m_pred = (preds_original*(0.5*preds_original.coords['d_bin_centers'])**m).sum(dim=('d_bin_centers'))
    try:
        m_label = test_moments.sel(moments=m)
    except KeyError:
        print('No direct moment data')
        print('Approximating moments from histogram data')
        m_label = (test_labels*(0.5*test_labels.coords['d_bin_centers'])**m).sum(dim=('d_bin_centers'))

...
wavelength = 355e-9
amplitude_th = 0.2  # minimum amplitude
...
for iholo in range(Nsets):
    pix_x = np.int(np.round(np.random.rand()*image_dim['x']+image_dim['x']/2))
    pix_y = np.int(np.round(np.random.rand()*image_dim['y']+image_dim['y']/2))
    adata = np.zeros((image_dim['x']*2,image_dim['y']*2))
    if binary_amplitude:
        # force amplitude data to be binary
        adata[pix_x,pix_y] =  1.0
    else:
        # let amplitude be any number between 0 and 1
        adata[pix_x,pix_y]  = np.random.rand()*(max(param_lim['amplitude'])-min(param_lim['amplitude']))+min(param_lim['amplitude'])
    ...
    zdata = np.zeros((image_dim['x']*2,image_dim['y']*2))
    zdata[pix_x,pix_y] = np.random.rand()*(param_lim['z'][1]-param_lim['z'][0])+param_lim['z'][0]
    ...
    E1.propagate_to(zdata[pix_x,pix_y])
    E1.field[pix_x,pix_y] *= (1-adata[pix_x,pix_y])

...
ds_name = "synthetic_holograms_v03.nc"  # 3 particle data
...
hist_bin_count = {"z":51,}
max_count = 10  # maximum number of particles expected
...
with xr.open_dataset(ds_path+ds_name) as ds:
    ...
    particle_count = xr.DataArray(np.zeros([ds['hologram_number'].size]+[max_count],dtype=np.int8),
                                coords={'hologram_number':ds['hologram_number'].values,'count':np.arange(max_count)+1},
                                dims=['hologram_number','count'])
    ...
    ds_ft = ds_ft.assign(particle_count=particle_count) # add the particle count to the data set
    ...
    for im in ds_ft['hologram_number'].values:
        ...
        ds_ft['particle_count'].loc[{'hologram_number':im,'count':particle_index.size}] = 1 
...
ds_ft.to_netcdf(ds_path+ds_base+"_"+
    "".join(hist_bin_count.keys())+"_"+
    "_".join(ft_func.keys())+"_"+
    encoded_dtype+"_histogram_count.nc",**nckwargs)

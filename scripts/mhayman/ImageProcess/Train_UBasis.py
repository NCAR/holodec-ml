...
if len(model_file) == 0:
    ...
    nn_descript = f'UNET_Basis_Layers{nLayers}_Conv{nConv}_Pool{nPool}_Filt{nFilters}_'+loss_str+'_'+out_act
    run_num = 0
...
all_labels = ds[input_variable]
scaler = ml.MinMaxScalerX(all_labels,dim=('xsize','ysize'))
scaled_all_labels = scaler.fit_transform(all_labels)

train_labels = scaled_all_labels.isel(hologram_number=slice(valid_index,split_index))
...
scaled_in_data = scaler.fit_transform(in_data)

...
from tensorflow.keras.layers import Input, Conv2D, Dense, Flatten, Activation, MaxPool2D, SeparableConv2D, UpSampling2D, concatenate, Conv2DTranspose, Reshape
...
if len(model_file) == 0:
    ...
    nn_descript = f'Manifold_Conv{nConv}_Filt{nFilters}_'+loss_str+'_'+out_act
    run_num = 0
...
if new_model:
    ...
    flat_in = Flatten()(cnn_input)
    d1 = Dense(16*scaled_in_data.shape[1],activation='tanh')(flat_in)
    d2 = Dense(16*scaled_in_data.shape[2],activation='tanh')(d1)

    reshape_d2 = Reshape(np.sqrt((16*scaled_in_data.shape[1],16*scaled_in_data.shape[2],1)))(d2)

    c1 = Conv2DTranspose(nFilters, (nConv, nConv),strides=(2,2),padding="same", kernel_initializer = kernel_initializer)(reshape_d2)
    c1a = Activation("relu")(c1)
    c2 = Conv2DTranspose(nFilters, (nConv, nConv),strides=(2,2),padding="same", kernel_initializer = kernel_initializer)(c1a)
    c2a = Activation("relu")(c2)

    # c1 = Conv2D(nFilters, (nConv, nConv), padding="same", kernel_initializer = kernel_initializer)(reshape_d2)
    # c1a = Activation("relu")(c1)
    # c2 = Conv2D(nFilters, (nConv, nConv), padding="same", kernel_initializer = kernel_initializer)(c1a)
    # c2a = Activation("relu")(c2)

    out = Conv2DTranspose(2, (7,7), strides=(1,1),padding="same")(c2a)

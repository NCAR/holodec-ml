...
def ddp_setup(rank, world_size):
    logging.info(f"Running basic DDP example on rank {rank} with world_size {world_size}.")
...
def trainer(rank, world_size, conf, trial=False):
    ...
    # during fit the fit function needs to know where to send the data, hence why the model needs to know its rank
    unet.device = rank 
    # have to send the module to the correct device first
    unet.to(device)
    unet = torch.compile(unet)
    # will not check that the device is correct here
    ddp_model = DDP(unet, device_ids=[f"cuda:{rank}"], output_device=None)
    # sharding the optimizer state
    #optimizer = ZeroRedundancyOptimizer(ddp_model.parameters(), optimizer_class=torch.optim.AdamW, lr=learning_rate)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for k, (inputs, y) in batch_group_generator:
            ...
            inputs = inputs.to(ddp_model.device, non_blocking=True)
            y = y.to(ddp_model.device, non_blocking=True)

            # Clear gradient
            optimizer.zero_grad()

            # get output from the model, given the inputs
            pred_mask = ddp_model(inputs)
            ...
            batch_loss = torch.Tensor([train_running_loss / counter]).cuda(ddp_model.device, non_blocking=True)
        ...
        with torch.no_grad():
            ...
            for k, (inputs, y) in batch_group_generator:
                ...
                # update tqdm
                batch_loss = torch.Tensor([valid_loss / counter]).cuda(ddp_model.device, non_blocking=True)
                dist.all_reduce(batch_loss, dist.ReduceOp.AVG, async_op=False)
        ...
        #man_loss = predict_on_manual(epoch, conf, ddp_model, device)  # + np.mean(batch_loss)
...
if __name__ == "__main__":
    ...
    #mp.spawn(demo_basic, args=(args.world_size,), nprocs=args.world_size, join=True)

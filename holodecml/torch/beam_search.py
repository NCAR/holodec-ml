...
class BeamSearch:
    ...
    def search(self,
        ...
        start_predictions = start_predictions.contiguous()
        ...
        # Added by John - return the probabilities for all steps rather than just the 
        # final step, which was the default implementation
        ...
        if self.per_node_beam_size > num_classes:
            raise OSError(f"Target vocab size ({num_classes:d}) too small "
        ...
        probabilities.append(start_top_log_probabilities)
        ...
        for timestep in range(self.max_steps - 1):
            # shape: (batch_size * beam_size,)
            last_predictions = predictions[-1].reshape(batch_size * self.beam_size).contiguous()
            ...
            #restricted_beam_log_probs_reshaped = reshaped_summed.gather(1, restricted_beam_indices)

            predictions.append(restricted_predicted_classes)
            probabilities.append(restricted_beam_log_probs)
            
            # shape: (batch_size, beam_size)
            last_log_probabilities = restricted_beam_log_probs
            
            #_batch_size, _beam_size = last_log_probabilities.shape
            #probabilities.append(last_log_probabilities.exp().reshape(_batch_size, _beam_size, 1))
            ...
            # shape: (batch_size, beam_size)
            backpointer = restricted_beam_indices.float() / self.per_node_beam_size
        ...
        reconstructed_probabilities = [probabilities[-1].unsqueeze(2)]
        ...
        for timestep in range(len(predictions) - 2, 0, -1):
            ...
            cur_probs = probabilities[timestep].gather(1, cur_backpointers).unsqueeze(2)
            reconstructed_predictions.append(cur_preds)
            reconstructed_probabilities.append(cur_probs)
        ...
        final_probs = probabilities[0].gather(1, cur_backpointers).unsqueeze(2)

        reconstructed_predictions.append(final_preds)
        reconstructed_probabilities.append(final_probs)
        
        # shape: (batch_size, beam_size, max_steps)
        all_predictions = torch.cat(list(reversed(reconstructed_predictions)), 2)
        all_probabilities = torch.cat(list(reversed(reconstructed_probabilities)), 2)

        return all_predictions, all_probabilities.exp()
